    ...
    this.parseAttach = function(atta)
    {
        ...
        for (var i=0; i < z.length; i++)
        {
            ...
            if ( z[i][0].replace(/[0-9]/g,"")==""  && z[i][2].replace(/[0-9]+/g, "")=="___")
            {
                ...
                var bs = base2cn[cln];
                if (bs!=null)
                   bs.style.borderRadius = (parseInt(ns[2])/2) + "px/" + (parseInt(ns[3])/2) + 'px';
                 
                imagelet2wh[cln] =  (parseInt(ns[2])/2) + "px/" + (parseInt(ns[3])/2) + 'px'; 
                q += "div." +  cln
...
function changepage(sel)
{
   ...
   {    
       ...
       if (ismakingtab == 1 && typeof(LaTexHTML) != 'undefined')
       {
            ...
            {
                LaTexHTML.formatele(allShapes[k].div);
            } 
       }
   }
...
function makemultmedia(link,urla)
{
    ...
    if (xs[0] == '1')
    {
        if (ismakingtab!=1)
        {
            ...
            return '<img src="' + link + '" width=' + xs[1] + "  height=50px >";
...
function showedit(num, x,y,w,h,tt) 
{
    ...
    + "<td><div id=an" + num + " style=\"background-color:#BBBBBB;width:24px;height:24px;border-radius:12px;lineheight:24px;text-align:center;vertial-align:middle;font-size:18px\">&#10021;</div></td>"
...
function done(num, W, H, enforce) 
{
    ...
    {     
        z = new Shape(z.num,z.words,z.urlas, z.shapename,z.x,z.y, W, H, z.fontsize, z.color, z.bcolor, z.fc, z.time,z.zindex,z.start,z.fontfamily,z.slope);
    }
    else
    {  
        
        z = new Shape(z.num,z.words,z.urlas, z.shapename,z.x,z.y, null, null, z.fontsize, z.color, z.bcolor,z.fc,z.time,z.zindex,z.start,z.fontfamily,z.slope);
    }
    ...
    if (!allShapes[num].isrect())allShapes[num].gooddim();
    if (b!=null)
    {  
       document.body.removeChild(b);
       numediting = -1;
    ...
    else if (haseditt && demohandle==null)
        saveit();
    allShapes[num].ellipseimg();
}
...
function Shape (num, words, urlas, shapename,x, y, w, h, fs, cl, bc, fc,tm,zIndex,starttime, fontfamily,slope)
{
  ...
  var s = ("words=" + words + '\nsp='+ shapename+ '\nx=' + x+ '\ny=' + y+ '\nw=' + w+ '\nh=' + h+ '\nfs=' + fs+ '\ncl=' + cl+ '\nbc=' + bc+ '\nfc=' + fc+ '\nzi=' + zIndex+ '\nor=' + starttime) ;
   ...
   this.isrect = function(){return this.shapename.indexOf("rect")>=0 ;}
   var tmp = ";box-shadow:" + ( boxshadow*3) + "px " + ( boxshadow*3) + "px " + ( boxshadow*6) + "px #666666";
   if ( boxshadow == 0 || this.shapename == 'diamond'|| this.shapename == 'ellipse') tmp = "";
   this.base.style.cssText = "position:absolute;border:0px;top:" + y +"px;left:" + x +"px;border:0px" + tmp +";font-size:" + this.fs + "px;font-family:" + this.fontfamily + ";transform:rotate(" + this.slope +"deg)";
   ...
   { 
      ...
      var  zz =   "border:0px #000 solid;z-index:4;overflow:visible;padding:0px 0px 0px 0px;margin:0px 0px 0px 0px"
          + ";font-size:"   + this.fontsize + 'px;font-family:' + this.fontfamily + ';color:' +  colors[this.color]   + textshadow;
      ...
      { 
          ...
          if (this.height !=null)
           hh =   this.height/2;
          zz +=  ';transform:rotate(' + this.slope + 'deg);position:absolute;left:' + Math.round(this.x + ww*(1-shink)) + 'px;top:' + Math.round(this.y + hh*(1-shink)) + "px";
      }
   ...
   { 
       ...
       if ((this.fc & 1) == 1) framecolor = bcolors[this.bcolor];
       var textshadow = "text-shadow:1px 1px #505050;";
       if ( (this.fc & 4) == 0) textshadow ='';
       ...
       { 
           // if ( (this.fc & 1) > 0) fw = 0;
            ...
            if ( (this.fc & 1)>0)  this.div.style.borderRadius = this.framew() + 'px';
       ...
       else if (this.shapename == 'ellipse')
       {
           ...
           this.div.style.transform = 'rotate(' + this.slope + 'deg)';
       }
       else
           this.div.style.transform = 'rotate(' + this.slope + 'deg)';
  ...
  this.makediamond  = function(w,h)
   {
      ...
      if (w == null) 
       ...
       var rt = w/h;
        var fw = 0;  if ((this.fc & 1)==0) fw = 2;
        var bs = ''; if ((this.fc & 2) > 0) bs = "box-shadow:3px  3px #ccc;-moz-box-shadow:3px  3px  #ccc;-webkit-box-shadow:3px -3px  #ccc;";
        var bt ='';  if ((this.fc & 1) == 0) bt = "border:2px " +   colors[this.color] + " solid;"
        return "<div style=\"border:0px;position:relative;height:" + h + "px;transform:scaleX(" + rt + ");transform-origin:0%;width:" + h + "px\"><div style=\"background-color:" + bcolors[this.bcolor] + ";" + bs + bt +"height:" + (h*100/141.42) + "px;width:" + (h*100/141.42) + "px;transform:rotate(45deg);transform-origin:35.5% 84%;\"></div></div>";
   ...
   { 
       ...
       var framecolor = colors[this.color];
       if ( (this.fc & 1)>0) framecolor = bcolors[this.bcolor];
       if (this.shapename== 'rightrect')
       {
           s = tohtml(this.words, this.urlas, colors[this.color], this.fontsize,this.shapename);
           if ( (this.fc & 1)>0) fw = 0; 
           var xx = "background-color:" + bcolors[this.bcolor] +";";
           if ( (this.fc & 8) > 0) xx = "background-image:" + gradient(bcolors[this.bcolor]) + ";"
  ...
  // if (editable && typeof(Drag)!='undefined') Drag.init(this.base);
   ...
   {
       if (this.shapename == 'ellipse'   && this.words.replace(new RegExp("\\[" + textmsg[1303] + "[0-9]+\\]","i"),'')=='')
       {
          this.div.innerHTML = '';
   ...
   else
   {
      ...
      // this.div.className = "samebg";
       ...
       this.div.innerHTML =  tohtml(this.words, this.urlas, colors[this.color],this.fontsize,this.shapename);
   ...
   this.init = function()
   {
       ...
       {
            if ( (sp.fc & 1)>0)  sp.div.style.borderRadius = sp.framew() + 'px';
       }
       sp.uniz();
       document.onmousemove = onmouseover0; 
   }
...
function Line(ordernum,type,startnum,sx,sy,endnum,ex,ey,thick,direct,tm,cl)    
{
   ...
   this.draw = function()
   {
        ...
        else if (this.direct >= 1)
        {
            ...
            {
                var l = 1; if (Ax > bx) l = -1;
                var ly = 1; if (by < Ay) ly = -1;
   ...
   this.drawlo = function(x0,y0,x1,y1,thick,mg)
   {
    ...
    else
    {
        ...
        if (mg!=null && mg!='')
        {
            ...
            dv.innerHTML = '<table cellpadding=0 cellspacing=0><tr height=' + f + " valign=middle><td width=" + (width-f) + " valign=middle><div style=\"margin:0px 0px 0px 0px;width:" + (width-f) + "px;height:" + thick + "px;background-color:" +  colors[this.color]  + "\"><!----></div></td><td style=\"color:" + (colors[this.color]) + "\" width=" + f + " align=center valign=middle>" + mg + "</td></tr></table>";
        }
        else
        {
            dv.style.cssText = 'z-index:0;position:absolute;left:'
            ...
            + top +  'px;width:'
            + (width) + 'px;height:'
            + thick + "px;background-color:" +  colors[this.color]  + ';-ms-transform: rotate(' + deg 
...
function mdia(num,cd )
{
       ...
       if (  cd== 1)
       { 
            str += "<tr><td><table align=center width=100%  cellspacing=0 cellpadding=0 border=1><tr>"; 
            str += "<td   valign=middle align=center width=25%  style=color:blue onclick=redraw3(" + num +   ",'rightrect'," + cd + ")><img width=10 style=\"border:1px black solid\" src=\"" + originalurl + "/image/rightrect.png\"></td>";
            str += "<td   valign=middle align=center width=25% ><img style=\"border:0px;width:13px;height:10px\"  src=\"" + originalurl +"/image/roundrect.png\" onclick=redraw3(" + num +   ",'roundrect'," + cd + ") > </td>";
            str += "<td   valign=middle align=center  width=25%   style=font-size:12px;color:blue  onclick=redraw3(" + num +   ",'ellipse'," + cd + ")>O</td>";
            str += "<td   valign=middle align=center  width=25%  style=font-size:14px;color:blue   onclick=redraw3(" + num +   ",'diamond'," + cd + ")>" + diamondchar +"</td>";
            str +="</tr></table></td></tr>";
       }
       else if (cd == 5)
       {
       ...
       if (cd== 1 || cd==5)
       {
            ...
            str += "<tr><td><table width=100% align=center cellpadding=0 cellspacing=0 border=1><tr height=14>";
            for (var i=0; i <  allfonts.length; i++)
            {     
...
function copyapage(xys)
{
   ...
   for ( j=0; j<N;j++)
   {
       if (allShapes[j].isrect())continue;
       x = document.getElementById('d'+j);
       if (x==null) continue;
