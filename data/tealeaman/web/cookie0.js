...
{
var someconsts=['<meta http-equiv="Pragma" content="no-cache" ><meta http-equiv="content-type" content="text/html; charset=gb2312" />','arial,KaiTi,KaiTiGB2312,STKaiti','121334211234341231231324354546765765sdffhgfjhgasczerrtredsv1312344dfdw3rhdsgQWERETEASA','1','3598000'];
var unititle = function(str,cls){
        if (cls!='outset2') 
        return '<table cellspacing=0 cellpadding=0 width=100% border=0 align=center class=' + cls + ' ><tr height=48><td valign=middle align=center><table width=100%  align=center><tr><td width=36 align=middle><img src=image/s8.png width=36 onclick=gomenu()></td><td  align=center><div class=forcurve1 style=width:180px;overflow:hidden  id=titlediv  onclick=qrlink()><NOBR>' + str + '</NOBR></div></td><td  onclick=actionmenu(this) valign=middle width=20  align=center  style=\"font-size:20px;color:white;padding:0px\">&vellip;</td></tr></table></td></tr></table>'; 
        return '<table  cellspacing=0 cellpadding=0 width=100%   align=center ><tr><td width="2px"></td><td class=outset2 valign=middle align=center style="border:1px outset"><table width=100% ><tr><td width=36 valign=middle><img src=image/s8.png width=36  onclick=gomenu()></td><td  align=center><div class=forcurve1  onclick=qrlink()  id=titlediv><NOBR>' + str + '</NOBR></div></td></tr></table></td><td width="3px"></td></tr></table>';
    } 
}
...
function formatstr(v, fmt)
{
    ...
    for (var r = 0;  r < x.length; r++)
    {
       ...
       {
           str += '<table align=' + (x[start+1][0].indexOf('  ')==0?'center':'left') + ' border=1 style="border:1px #aaaaaa solid;border-collapse:collapse;border-radius:3px;border-color:#cccccc">';
           for ( var i= start+1; i <= r; i++)
           {
               str += "<tr>";
               var startj = 0;
...
{
    if (elenametxt==null || formhandle == null || formhandle.tagName.toLowerCase() != 'form')
        return null;
...
{
    var b = gomenu0();
    if (b) myprompt(textmsg[345] + ' ' + textmsg[107] + ' tab' )
    else
    {
...
function gomenu0()
{
    if ( document.title==textmsg[107])
        return true;
    else
    {
        var father = parent.parent.opener;
        if (father==null || typeof(father.gomenu0)=='undefined')
        {
            return false;
        }
        else
        {
            return father.gomenu0();
        }
    }
...
function qrlink()
{
    ...
    if (typeof(theurl) == 'undefined' &&  parent != window)
    {
        ...
        window.open("Qrlink?url=" + Msg.hex(theurl1), '_blank');
    }
    ...
    {
        myprompt("<img src=\"image/qrcode.gif\" >",null,null);
    }
}
...
function setbackf(x,err)
{
    ...
    if(N >= 2)
    {
        ...
        myprompt('<form rel=opener name=fef  ><font color=red>(' + (++numofuploadtries)  + ')</font> The following records failed to match. Match them manually by selecting columns then <input class=GreenButton type=button onclick=document.thisform.submit() value=Resubmit><table border=1 style=border-collapse:collapse>' + err1 + "</tr>" + err + '<table><form>');
...
function recurainput(x)
{
   ...
   if (typeof(x.tagName) !='undefined' && x.tagName.toLowerCase() == 'a' ) 
   {
      if (x.href.indexOf("javascript")>=0)
          allainputeles += "<tr><td onclick=\"" + x.href +";closeactmenu()\"><nobr>" + x.innerHTML + "</nobr></td></tr>";
      else
          allainputeles += "<tr><td onclick=\"open('" + x.href +"','_blank');closeactmenu()\"><nobr>" + x.innerHTML + "</nobr></td></tr>";
   }
   else if (typeof(x.tagName) !='undefined' && x.tagName.toLowerCase() == 'input' && x.type.toLowerCase() =='submit') 
   {
      allainputeles += "<tr><td onclick=\"" + (''+x.onclick).replace(/^[^\{]+/,'').replace(/ /g,'') +";closeactmenu()\"><nobr>" + x.value + "</nobr></td></tr>";
   }
   ...
   {
      allainputeles += "<tr><td  onclick=\"" + (''+x.onclick).replace(/^[^\{]+/,'').replace(/ /g,'') +";closeactmenu()\"><nobr>" + x.innerHTML + "</nobr></td></tr>";
   }
...
function actionmenu(spn)
{
    ...
    var str = "<table width=80 >" + allainputeles +  "<tr><td onmouseout=closeactmenu() onclick=closeactmenu()>" + textmsg[1824] +  "</table>";
    var xy = findPositionnoScrolling(spn);
    var dv = document.createElement("div");
    dv.id = 'dotmenu';
    //dv.className = 'outset2';
    dv.style.cssText = "background-image:" + gradientbg + ";background-position:-30px 0px;background-color:#222222;color:white;width:80px;position:absolute;top:" + xy[1] + "px;left:" + (xy[0]-60) 
...
eventcodes[37] = "if(cc==1){if(valuechanged[rr])for(var i=0;i<numCols;i++)if(ele(rr,i).value=='')setv(rr,i,defaultRecord[i]);}else if (cc==6&&rdapname=='roster')setCell(rr,7,''+(parseInt(retrv(rr,7))+parseInt(v)-parseInt(x)));";
eventcodes[38] = "if(mat[0][8]=='')setv(0,8,mat[0][14]);if(mat[0][5]=='')setv(0,5,mat[0][15]);showtimeslots(ele(0,3),3,1)";
eventcodes[39] = "parent.resizeIframe(0,(2+NUMROWS)*(font_size+8)+20)";
...
eventcodes[51] = "parent.match();parent.resizeIframe(1,12+(NUMROWS+3)*33)";
...
eventcodes[56] = "parent.regroup();parent.resizeIframe(1,12+(NUMROWS+3)*33)";
