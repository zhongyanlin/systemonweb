...
function seeanswers(i,jj,sname1)
{
    ...
    for (var k=1; k <= maxqn; k++)
    {
        ...
        {
            zz = vv.replace(/`([0-9]+)``([0-9]+)@/g,'<span id=g'+ k + '_$2 style=float:right;color:blue  onclick=adapteAsAnswer(' + i + ',' + k + ',$2)><$1></span><br>');
        } 
        else
...
function reformat(sel, i, k)
{
    ...
    {
        zz = vv.replace(/`([0-9]+)`/g,'<span style=float:right;white-space:nowrap><$1><span style=color:blue;transform:rotate(90deg)  onclick=adapteAsAnswer(' + i + ',' + k + ',' +'$1)>$</span></span><br>');
    } 
    else
