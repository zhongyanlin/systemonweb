...
function newa(i,td)
{
    ...
    {
      /*  myprompt( "(1)" + therow(i).cells[2].getElementsByTagName('select')[0].options[0].value + "<br>"
        + "(2)" + therow(i).cells[3].innerHTML + "<br>"  
        + "(3)" + therow(i).cells[4].innerHTML + "<br>" );*/
        let tbl = document.getElementById('absencetbl');
        
        let str ="<table><tr><td>" + tbl.rows[0].cells[2].innerHTML + "</td><td><select id=\"leavetime\" style=color:blue;font-size:16px;font-weight:700 onchange=\"goodtime(this)\"><option>"
        + option0 + "</option>";
  ...
  //  var sel = document.getElementById('leavetime');
    for (var l=0; l < 4; l++)
    {
        for (var i=0; i < times.length; i++)
        {
            ...
            if (k < p) continue;
            k = Math.round(k/1000);
            str += "<option value" + k + ">" + timestr(k) + "</option>";
            //sel.options[m++] = new Option(timestr(k), k);
       }
    }
    str += "</select></td></tr>"
    str += "<tr><td valign=top>" + tbl.rows[0].cells[3].innerHTML + "</td><td><textarea  id=\"prompttxt\" rows=7 cols=70>e.g. sick</textarea></td></tr>";
    str += "<tr><td>" + tbl.rows[0].cells[4].innerHTML + "</td><td style=\"color:blue;border-left-width:0px;border-right-width:0px\"  onclick=\"attachmore(document.getElementById('absencetbl').rows.length-2,this)\">";
    str += "[+]<input type=\"hidden\" value=\"\" id=\"att"  + (tbl.rows.length-2) + "></td></tr></table>";
     myprompt(str, null,null,tbl.rows[0].cells[1].innerHTML);
     var xy =findPositionnoScrolling(tbl);
...
syn = function(x,y)
{
    ...
    therow(activei).cells[7].innerHTML = textmsg[237];
    postopen("checkgrades.jsp", 
