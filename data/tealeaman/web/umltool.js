...
if (folder == null || folder == '') folder = prompt("working folder in your web home folder","");
}
var subdb = null;
function rcolor(x)
{
...
{
    var str = localStorage["UMLtoolstr"];
...
function makefile()
{
    ...
    var xx = "";
    if (tx == null);
    else if (tx.indexOf('http') == 0 && tx.length>15)
        xx = ";background-image:url(" + tx +")";
    ...
    s += "\n/*1*/jsscripts = ['en.js','cookie.js','checkHTML.js','attachment.js','curve.js?sn=30&dn=20','findrep.js','installtool.js','mousetrap.min.js','umltool.js'];\n";
...
function savetocache()
{
    ...
    {
       var str = toString();
       localStorage["UMLtoolstr"]  = "" + ((new Date()).getTime()) + " " + filename + " " + pagenum + " " + str;
    }
}
...
Msg.handlepost = function(s)
{
    ...
    else if (m.code == 'snap')
    {
        ...
        var tbl0 =  $('snap');
           ...
           var c = r.insertCell(-1);
           c.colSpan = 6;
           c.innerHTML =  ("<table width=98%><tr><td>" + x[0] +seg[1]+ "</td></tr> <tr><td>" + x[1] +seg[2]+ "</td></tr></table>");
...
Msg.handleget = function(s)
{
    ...
    else  if (m.code == "plain")
    {
        ...
        if (promptwin!=null && promptwin.innerHTML.indexOf(textmsg[1771].split(/@/)[6])>0)
            closeprompt();
        try{eval( m.msg );}catch(e){myprompt(m.msg,null,null,"Error");}
        Msg.needmore = true;
    }
    else  if (m.code == "leave")
    {
        Msg.send({code:'quit',msg:tstmp});
        Msg.needmore = false;
        usingremote = false;
...
var Play =
{
    ...
    endRemote: function ()
    {
        Msg.send({code: 'quit', msg: Msg.tid});
    },
    quitremote: function ()
    {
...
var onloadbeforeumlt = null;
if (typeof window.onload == 'function')
    onloadbeforeumlt = window.onload;
 
window.onload  = function()
{
    onlinetoolbarhas = true;
    if (onloadbeforeumlt!=null) 
        onloadbeforeumlt();
