...
function register()
{
    myprompt("<span style=color:red id=regerror></span><form name=f method=post action=go.jsp target=\"w" + tstmp + "\" onsubmit=\"return validate(this)\"><table width=300><tr><td>" + intfcwds[59] + "</td><td><input name=username class=text></td></tr>"
    + "<tr><td>" + intfcwds[60] + "</td><td><input  name=password  class=text type=password></td></tr><tr><td>" + intfcwds[61] + "</td><td><input name=email  class=text></td></tr>"
    + '<tr><td><img src="patchca.png" alt="captcha"  style="cursor:pointer;vertical-align:text-bottom;line-height:22px;height:22px;margin:0px 0px -3px 0px" onclick="randomit(this)">'
    +'</td><td><input type="text" class=text name="patchcafield" ></td></tr><tr><td colspan=2 align=center><input class=OrangeButton type=submit style=width:70px value="' + intfcwds[58] + '"></td></tr></table></form>',null,null,intfcwds[58]);
}
...
function setcolor(d,clr)
{
    ...
    {
        d.innerHTML = '<img src=image/' + clr + 'stone.png style=border:0px;width:' + STONESIZE + 'px >';
...
{
    if(btn.value == intfcwds[13])
    {
        holdstatus = status;
        setstatus(intfcwds[17]);
        btn.value = intfcwds[18];
    }
    else 
    {
        setstatus(holdstatus);
        btn.value = intfcwds[13];
    }
}
function draw(n)
{
...
document.getElementById('a').style.height = (board.offsetHeight+5) + 'px';
...
function alongborder(a,i,j, maincolor)
{
    ...
    if (goway == 2)
    {
        ...
        for (var   k=0; k < a.length; k++)
        {
            ...
            if (u[0] == -1 && u[1] == 0) //left
                v = [0, -1]; //top
            else if (u[0] == 0 && u[1] == 1)//bottom
                v = [-1, 0];  //left
            else if (u[0] == 1 && u[1] == 0) //right
                v = [0, 1];   //bottom
            else                            //top
                v = [1, 0];                 //right
...
function second5time()
{
    ...
    {
        t = Math.round(t/1000); var mi = Math.round(t/60);
...
function slow()
{
    ...
    x +='</select><br><br>Way:<select name=way onchange=getway(this)  style=width:200px ><option value=0 ' + (goway==0?'selected':'')
            ...
            + (goway==3?'selected':'') + '>breadth-first</option></select></div> '
...
function formreq(m,needmatch)
{
    ...
    var y = wishes.innerHTML.replace(/selected/ig,'').replace(/[^<]+/,'').replace(/<select /ig, '<select onchange="negotiate(this)"  ').split(/,/);
    if (needmatch==false)
     y = wishes.innerHTML.replace(/selected/ig,'').replace(/[^<]+/,'').replace(/<select /ig, '<select "  ').split(/,/);
    ...
    {   
        jj++;
        s += y[i].replace(/and /,'').replace(new RegExp('value="' + u[i] + '"'), 'value="' + u[i] + '" selected ');
    }
    if (jj > 2)
    {
    ...
    if (needmatch) s += "(" + intfcwds[55] + ":" + p +"/6)";
    s =  s.replace(/,[ ]*$/,'');
...
function gameroom()
{
    Msg.send({code:'plain',msg:'>>>snapshot',rid:''});
}
Msg.handlepost = function(s)
{
    ...
    {
        myprompt('<iframe width=300 height=250 src=login.jsp?follow=' + m.msg + ' />');
    }
    else if (m.code =='newd')
    {
        Msg.set('tid',m.tid);
...
function makecontest(m)
{
   ...
   (modified? intfcwds[30]:intfcwds[31])+ "\" onclick=\"acceptornegotiate('" + m.toString() + "',this)\">";
    return '<form name=resp' + (numreponses++) +  ' id="f' + m.sid + '" style="margin:3px 1px 3px 1px"><nobr>' + m.sname + " " 
    +  formreq(m,true) + a + "</nobr></form>";
}
...
Msg.handleget = function(s)
{
    ...
    else  if (m.code == 'latex')
    {
       ...
       {
            var y = '<form name=neg'   +  ' style="margin:20px 0px 20px 0px"><nobr>' +  m.sname + ' '+ intfcwds[56] + ':<br> "<b>' + Msg.sname
            + formreq(m,false) +   ("\"</b><br><center><input name=acceptbtn class=OrangeButton type=button style=\"width:100px\" value=\"" + intfcwds[31] + "\" onclick=\"acceptnegotiate('" + m.toString()+ "',this)\">"
       ...
       else if (status == intfcwds[29]  && m.msg.indexOf('C')>=0 )
       {
           ...
           {
               document.getElementById(negotiatingform).innerHTML = document.getElementById(negotiatingform).innerHTML.replace(/match score/, '<b>'+intfcwds[50] + '!</b> match score');
           }
       }
    } 
    else  if (m.code == 'html')
    {
       if (status == intfcwds[24] || status == intfcwds[29])
       {
          acceptme(m);
    ...
    else  if (m.code == 'plain')
    {
        ...
        else if (m.msg.length % 4==0 && m.msg.replace(/0-9/g,'').replace(/a/g,'').replace(/b/g,'').replace(/c/g,'').replace(/d/g,'').replace(/e/g,'').replace(/f/g,'') == '')
        {
            restore1(m.msg);
        }
    }
    else if ( m.code.indexOf('new')==0)
    {
...
function acceptyou(m)
{
   ...
   if (x[3]=='1' ||  x[3]=='0' && (new Date()).getTime()%2 == 1) 
   {
      setcolor(document.getElementById('hisclr'), BLACK );
      setcolor(document.getElementById('myclr'), WHITE );
      mycolor = WHITE;
...
var onloadbeforego = null;
if (typeof window.onload == 'function')
{
   onloadbeforego = window.onload; 
}  
window.onload = function()
{
        if (onloadbeforego != null) onloadbeforego();
...
function doupload(fn)
  ...
  function help()
  {
      gogamehelp = gogamehelp.replace(/<br>/g,'');
