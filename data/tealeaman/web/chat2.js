function Message(tid, sid, sname, rid, code, msg)
{
    ...
    {
        return this.num + " " + this.serial + " " + this.tid + " " + this.sid + " " + this.sname.replace(/ /g, '_') + " " + this.rid + " " + this.code + " " + this.msg;
    }
}
...
var Chat = 
{
    ...
    init: function(pagekey, sid1, sname1, token, init0)
    {
        ...
        {
            Chat.xmlhttp = new XMLHttpRequest();
        }
        else
        {
             Chat.xmlhttp = new ActiveXObject("Microsoft.XMLHTTP");
        }
 
         Chat.xmlhttp.onreadystatechange = function()
        {
            if ( Chat.xmlhttp.readyState == 4 &&  Chat.xmlhttp.status == 200 && Chat.xmlhttp.responseText.length>20)
            {
                var j = handleasyn( Chat.xmlhttp.responseText);
                Chat.listen();
    ...
    trim: function(x)
    {
        ...
        if (i >= j + 1)
            return '';
        return x.substring(i, j + 1).replace(/<[^>]*>/g, '');
    },
    ...
    updatetopicnp: function(topic, n, s)
    {
        ...
        if (i < tbl.rows.length)
        {
            ...
            else if (n == '0' && !Chat.ismycourse(topic))
            {
                tbl.deleteRow(i);
            }
            else
    ...
    makecell: function(topic, subdb, k)
    {
        ...
        c.innerHTML = round1('') + "<table   border=0  class=outset1><tr><td class=titlecell align=center>" + unititle(topic, 'outset2').replace(/style="/, "style=\"text-shadow:#707070 1px -1px;") + "</td></tr>"
                + "<tr><td><form name=\"t" + k + "\" style=\"margin:0px 0px -2px 0px\"> "
                + "<TABLE   border=0 class=outset1  align=center width=100% cellpadding=0 cellspacing=0>"
                + "<TR><TD   valign=TOP aling=center>"
                + "<textarea   name=msg onfocus=\"Chat.setcurrent(" + k + ");onlinetoolbarfollow(this)\" onkeypress=\"return mkstrike1(this,event)\"  onblur=Chat.setfmt()>"
                + "</textarea> </td> <td valign=top align=center id=act" + k + "><table align=center border=1 cellpadding=0   style=\"width:" + fs + "px;margin:0px 0px 0px 0px;display:block;background:white;border:1px #b0b0b0 outset;border-radius:4px;border-collapse:collapse\" cellspacing=0 cellpadding=0 valign=top >"
                + "<tr><td colspan=2  align=center id=timeout" + k + "  >0h0':0\"</td></tr>"
                + "<tr><td colspan=2  align=right><select  style=\"background-color:white;border:0px\" name=status onchange=Chat.changestatus(" +   k + ",this)>" + statoption + "</select></td></tr>"
                + "<tr><td align=left valign=middle  id=cls" + k + " style=\"width:" + (fontsize+4) + "px;color:blue;cursor:pointer;font-family:arial;font-weight:bold;font-size:" + (fontsize-1) + "px;text-align:center;border:0px\" onclick=Chat.selColor(" + k +",this)>A<!--5--></td><td align=right valign=middle><select name=fontszie style=\"background-color:white;border:0px\"  onclick=Chat.setfs(" + k + ",this)>" + v + "</select></td></tr>"
  /*            
   * 
   * 
   * 
...
*/      
                ...
                + "</table><div style=height:5px;width:40px;display:block></div><input   type=hidden name=fmt value=0><input   " + Chat.butstyle.replace(/"/,'"height:' + (3.5*fontsize) + 'px;border-radius:4px;') + "  class=OrangeButton  align=right  onclick=Chat.send(" + k + ") value=\"" + textmsg[223] + "\">"
        ...
        //resizebut(Chat.formbyname("t" +k));
        //resizebut(Chat.formbyname("f" +k));
        unifonts(c,myfontname);
        
        return k;

    },
    chgpolicy : function(ck)
    {
        var i = ck.checked?1:0;
        
        var session = Chat.byorder(Chat.activek);
        var xx = session.msg.value;
        session.msg.value = "";
        Chat.sendfm(">>>policy" + i); 
    ...
    status : function(k)
    {
        ...
        var xx = session.msg.value;
        session.msg.value = "";
        Chat.sendfm(">>>snapshot"); 
        session.msg.value = xx;
    },
    loadfromdb : function (k, err, l0, l1)
    {
       Chat.activek = k;
       if (l1 ==null) l1 =   Math.floor( (new Date()).getTime()/1000);
   ...
   loadfromdb1 : function (v)
   {
       ...
       document.form3.purpose.disabled = true; 
       document.form3.starttime.value = l0;
       document.form3.endtime.value = l1;
       var session = Chat.byorder( k);
       document.form3.target = "w" + tstmp;
       document.form3.courseid.value = session.courseid;
       document.form3.subdb.value = session.iid;
...
document.form3.submit();
    ...
    savetodb : function(k)
    {
        ...
        var k=0;
        var as = [];
        Chat.deletepics(session.recev,as);
        var a = '';
        for (var l=0; l < as.length; l++)
            a += as[i] + ",";
        session.deletemath(session.recev);
        var loc = document.location.toString().replace(/talkpage*$/,'').replace(/:/,".").replace(/\//g,".") + "FileOperation";
        var r = new RegExp(loc, "ig");
        document.form3.content.value =  session.recev.innerHTML.replace(r, "FileOperation");
        var i= document.form3.content.value.lastIndexOf( "<!-- -->");
        if (i > -1)
            document.form3.content.value = document.form3.content.value.substring(i+14);
        document.form3.courseid.value = session.courseid;
        document.form3.subdb.value = session.iid;
        document.form3.target = "w" + tstmp;
        //document.form3.target = '_blank';
        document.form3.attach.value = a;
...
document.form3.submit();
 ...
 f.submit();
    ...
    send: function(k)
    {
        ...
        if (v - Chat.lastlisten > TIMEOUT - 2000 || Chat.sessions[k].state == 'timeout')
        {
            Chat.listen();
        }
        ...
        if (session.msg.value != '')
        {
            ...
            var m = new Message(session.tid, Chat.sek, Chat.sname, session.sel()[0], Chat.formats[t], session.msg.value);
            session.msg.value = '';
            editingfmt = 0;
            session.setformat(0);
            closeprompt();
    ...
    {
        var session = Chat.byorder(k);
        Chat.post(new Message(session.tid, Chat.sek, Chat.sname, session.sel()[0], 'status', sl.options[sl.selectedIndex].value));
    },
    ...
    {
        var xy = findPositionnoScrolling(btn);
        Chat.byorder(k).panel([xy[0] - 240, 50]);
        promptwin.style.top = (xy[1] + 30) + 'px';
        promptwin.style.left = (xy[0] - Math.round(promptwin.offsetWidth/2-35)  ) + 'px';
    },
    ...
    {
        var sel = Chat.bytid(tid).sel();
        if (sel[0] != '')
        {
            Chat.bytid(tid).fired(sel[0]);
            Chat.post(new Message(tid, Chat.sek, Chat.sname, sel[0], "fire", ""));
        }
        if (sel[1] != '')
        {
            Chat.bytid(tid).unfired(sel[1]);
            Chat.post(new Message("0", Chat.sek, "chat", sel[1], "fire", tid));
        }
    },
    block: function(td, tid)
    {
        var sel = Chat.bytid(tid).sel()[0];
    ...
    {
        var v = (new Date()).getTime();
        var url = "Msgrecev?" + ((Chat.swich++) % 100) + "&sek=" + Chat.sek + "&sid=" + Chat.sid + "&app=chat&time=" + v;
        if (timeoutInMin != null)
            url += "&timeout=" + timeoutInMin;
        
        Chat.xmlhttp.open("GET", url, true);
        Chat.lastlisten = v;
        Chat.xmlhttp.send();
    ...
    post: function(m)
    {
        ...
        if (m.rid != null && m.rid != '')
        {
            ...
            Chat.fsnd.rid.value = ("," + m.rid.replace(/ /g,'').replace(/^[,]+/,"").replace(/[,]+$/,"") + ",").replace(new RegExp(","+ Chat.sek + ","), ",").replace(/^[,]+/,"").replace(/[,]+$/,"");
        }
        else
...
document.form3.submit();
    ...
    this.init = function(s)
    {
        ...
        this.msg = Chat.fele("t" + this.order, "msg");
    ...
    this.memberlist = function(t)
    {
       ...
       for (var i = 0; i < ss.length; i++)
       {
          ...
          {
               this.member[this.size++] = [aa, bb, textmsg[1304].replace(/@.*$/,''), tt, 'blue'];
               allids += aa + ",";
          }
        } 
    }
    ...
    this.add = function(sid, sname)
    {
        ...
        this.member[this.size++] = [sid, sname, textmsg[1304].replace(/@.*/,''),tt, 'blue'];
    ...
    this.printsent = function(m)
    {
        ...
        ResizeUploaded.changingid = '';
        if (  //m.serial==0 || 
          m.msg==null || m.msg.indexOf('!mycolor:') == 0)
            return;
    ...
    this.panel = function(xy)
    {
        var firable = Chat.updatemenu;
        var s = "<table width=100% id=p" + this.tid + ">";
        for (var i = 0; i < this.size; i++)
        {

            s += "<tr><td><input type=checkbox value=\"" + this.member[i][0] + "\"></td><td width=150><a href=DataFormHTML?subdb=&rdap=userinfo&uid=" + this.member[i][1].replace(/\-.*/,'') + " target=_blank><nobr>" + this.member[i][1].replace(/[^\-]+\-/,'') + "</nobr></a></td>"
                    + "<td width=90><nobr>" + this.member[i][2] + "</nobr></td></tr>";
        }
        s += "<tr><td><input type=checkbox onclick=Chat.checkall(this)></td>";
        if (firable)  //<table align=center><tr><td>
            s += "<td align=center colspan=2><input  class=RedButton  " + Chat.butstyle + " style=height:" + (fontsize + 6) + "px  onclick=Chat.fire(this,'" + this.tid + "') value=\"" + textmsg[1314]+ "\"></td>";//<td><input id=anchor" + this.order + " class=RedButton " + Chat.butstyle + " onmouseup=Chat.moveto(this," + this.order + ") value=\""+ textmsg[1315]+ "\"></td></tr></table></td>";
        else
            s += "<td colspan=2 align=center><input class=OrangeButton " + Chat.butstyle + "  style=height:" + (fontsize + 6) + "px  onclick=Chat.block(this,'" + this.tid + "') value=\""+ textmsg[1316] + "\" ></td>";
        s += "</tr></table>";
        myprompt(s, null, null, textmsg[1271]);
        promptwin.style.left = xy[0];
    ...
    this.append = function(str, fmt, cl)
    {
        ...
        if (fmt == 0)
        {
            ...
            this.recev.innerHTML  =  this.recev.innerHTML  
             +"<div style=color:" + cl + ";font-family:inherit;display:block>" + str.replace(/^[\n|\t|\r| ]+/,'').replace(/[\n|\t|\r| ]+$/,'').replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/\n/g, '<br>') 
             +  "</div>" ;
        }
        else
        {
            ...
            if (fmt == 1)
            {
                if (cl==null || cl=='') cl = document.getElementById("cls" + Chat.activek).style.color;;
                this.recev.innerHTML  = this.recev.innerHTML  
                        + "<div style=color:" + cl + ";font-family:inherit;display:block>"
    ...
    this.sel = function()
    {
        var tbl = document.getElementById('p' + this.tid);
        if (tbl == null)
            return ['', ''];
        ...
        for (var i = 0; i < tbl.rows.length; i++)
        {
            ...
            {
                if (this.member[i][2] != textmsg[1570])
                {
                    if (s != '')
                        s += ",";
                    s += this.member[i][0];
    ...
    this.startuml = function(initial)
    {
        ...
        if (win == null)
        {
            win = myprompt("<iframe  name=\"ifrnam" + this.order + "\"  id=\"ifr" + this.order + "\" src=umltool.html?sn=" + this.order + "&en="+ encoding + " width=560 height=600 frameborder=0 style=\"border:0px;margin:0px 0px 0px 0px\" on></iframe>",null,null,this.topic,"chatdraw" + this.tid);
            ...
            {
                var m = new Message(this.tid, Chat.sek, Chat.sname, this.sel()[0], 'draw', '');
                Chat.post(m);
            }
...
function handleasyn(s)
{
    ...
    var session = null; 
    if (m==null || typeof(m.num) == 'undefined' || m.valid == false)
    {
        return 0;
    }
    else
    ...
    if (m.code == 'login')
    {
        myprompt("<iframe width=300 height=240 src=login.jsp?error=generic&follow=" + m.msg + " />");
    }
    ...
    else if (m.code == "new")
    {
        ...
        if (topic.indexOf("-") <= 0 || Chat.ismycourse(topic))  
    ...
    else if (m.code == "sure0" || m.code == "sure1")
    {
        session.printsent(m);
        Queue.sending = false;
        if (Queue.size>0)
    ...
    {
        var seg = m.msg.split(/\|/);
        var x = textmsg[1646].replace(/@/g, '</nobr>@<nobr>').split(/@/);
        for (var i=1; i < 4; i++)
        ...
        myprompt("<table width=98%><tr><td>" + x[0] +seg[1]+ "</td></tr> <tr><td>" + x[5] +seg[2]+ "</td></tr><tr><td>" + x[13] +seg[3]+ "</td></tr></table>");
    }
    else if (m.code == "policy" && m.msg!='')
    {
        if (m.msg == '1') m.msg  = textmsg[1646].replace(/.*@([^@]+)$/,"$1");
        else m.msg =  textmsg[1357]
    ...
    if (m!=null && typeof(m.num) !='undefined')
        return m.num;
    return 0;
}
var chatoldonload  = null;
if (typeof window.onload == 'function')
   chatoldonload = window.onload;
var helpcloser = null;
...
onload = function()
{
    ...
    Chat.init(pagekey, userid, username, securitytoken, initialtopics);
    ...
    if (chatoldonload!=null) chatoldonload(); 
    
    if (parent.opener!=null && typeof(parent.opener.closeallchat)!='undefined')
    {
        helpcloser = parent.opener.closeallchat;
         
    }
    else if (typeof(parent.frames[0].closeallchat)!='undefined')
    {
        helpcloser =     parent.frames[0].closeallchat;
    }
...
var Queue = 
{
    ...
    sendnow : function()
    {
        ...
        var m = new Message(session.tid, Chat.sek, Chat.sname, session.sel()[0], 'draw', s);
